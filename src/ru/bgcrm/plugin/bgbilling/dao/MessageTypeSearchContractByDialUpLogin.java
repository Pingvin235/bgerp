...
public class MessageTypeSearchContractByDialUpLogin
	extends MessageTypeSearchBilling
{
	private static final Logger log = Logger.getLogger( MessageTypeSearchContractByDialUpLogin.class );

	private int moduleId;
	
	public MessageTypeSearchContractByDialUpLogin( ParameterMap config )
    	throws BGException
    {
    	super( config );
		...
		throws BGException
	{
		...
		List<DialUpLogin> searchResult = dialUpDao.findLogin( login, Utils.parseInt( login ) > 0 ? DialUpDAO.FIND_MODE_LOGIN : DialUpDAO.FIND_MODE_ALIAS );
		
		for( DialUpLogin object : searchResult )
		{
			result.add( new CommonObjectLink( 0, Contract.OBJECT_TYPE + ":" + billingId, object.getContractId(), 
			                                  object.getContractTitle(), 
			                                  object.getLogin() + " " + object.getAlias() ) );
		}
