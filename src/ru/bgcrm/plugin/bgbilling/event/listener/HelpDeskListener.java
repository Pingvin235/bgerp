...
public class HelpDeskListener
{
		...
		if( pair == null )
		{
			for( MessageTypeHelpDesk mt : getMessageTypes() )
			{
				if( paramId == mt.getStatusParamId() ||
					paramId == mt.getCostParamId() )
				{
					throw new BGMessageException( "Данные параметры разрешено править только для процессов, связанных с HelpDesk темами." );
				}
			}			
			return;
		}
	...
	private void processChanged( ProcessChangingEvent e, ConnectionSet conSet )
    	throws BGException
    {
    	...
    	else if( e.isExecutors() )
    	{
        	...
        	{   
        		...
        		if( executors.size() > 1 )
        		{
        			throw new BGMessageException( "Для процесса связанного с HelpDesk запрещена установка более одного исполнителя с ролью 0." );
        		}
    	...
    	if( e.isStatus() )
    	{
        	...
        	if( pair != null )
        	{
				...
				if( mt.getMarkMessagesReadStatusIds().contains( e.getStatusChange().getStatusId()  ) )
        		{
        			...
        			messageDao.searchMessageList( searchResult, e.getProcess().getId(), mt.getId(), Message.DIRECTION_INCOMING, null, null,null,null,null );
