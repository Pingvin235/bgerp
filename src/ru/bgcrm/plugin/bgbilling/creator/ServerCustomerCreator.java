...
{
	private static final Logger log = Logger.getLogger( ServerCustomerCreator.class );
	...
	public ServerCustomerCreator( Config config, ParameterMap params )
	    throws BGException
	{
		this.config = config;
	...
	public void createCustomer( String billingId, Connection con, int contractId, int customerId )
	    throws BGException
	{
		try
		{
	...
	public void createCustomer( int contractId, String title )
	    throws Exception
	{
	...
	private Map<Integer, String> getContractParamValues( int contractId )
	    throws Exception
	{
	...
	private Customer findCustomerByParamsWithTitleConfirm( String customerTitle, int contractId, Map<Integer, String> paramValues )
	    throws Exception
	{
	...
	private void linkCustomer( Customer customer, int contractId, String contractNumber, Map<Integer, String> paramValues )
	    throws Exception
	{
		log.info( "Linking contract: " + contractId + " to customer: " + customer.getId() );

		// привязка договора к контрагенту
		CommonObjectLink link = new CommonObjectLink( Customer.OBJECT_TYPE, customer.getId(),
		                                              "contract:" + dbInfo.getId(), contractId, contractNumber );
		...
		for( Parameter param : config.importParameterList )
		{
			...
			for( int billingParamId : mapping.billingParamIdList )
			{
				...
				else if( Parameter.TYPE_ADDRESS.equals( param.getType() ) )
				{
					...
					{
						addressExist = value.getHouseId() == billingAddr.getHouseId() && value.getFlat().equals( billingAddr.getFlat() ) && value.getRoom().equals( billingAddr.getRoom() );
						if( addressExist )
						{
							break;
						}
