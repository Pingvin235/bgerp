...
public class CerberCryptPacketListExtensionFunction
	extends CommonExtensionFunction
{
	private User user;
	
	public CerberCryptPacketListExtensionFunction( Connection con, User user )
    {
	    super( con, "bgbilling-cerberCryptPacketList" );
	    this.user = user;
    }
		...
		return new ExtensionFunctionCall()
		{
			    ...
			    throws XPathException
			{
				try
                {
					...
					int moduleId = (int)((IntegerValue)arguments[0].next()).longValue();
					
					Document doc = new CerberCryptDAO( billingId, user ).getPacketList( moduleId );
