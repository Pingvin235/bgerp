...
public class ProcessCommandExecutor
{
		...
		throws Exception
	{
		...
		for( String command : commands )
		{
			...
			if( command.startsWith( "addGroupsInRole" ) )
			{
				...
				{
					addingProcessGroups = new HashSet<ProcessGroup>( CollectionUtils.intersection( addingProcessGroups, type.getProperties().getAllowedGroups() ) );
				}
			...
			else if( command.startsWith( "addExecutorsInGroupsInRole" ) )
			{
				...
				ProcessAction.processExecutorsUpdate( form, con, process,
													  Collections.singleton( processGroup ), executors );
			}
			...
			else if( command.startsWith( "newsNotifyExecutors" ) ||
					 command.startsWith( "newsPopupNotifyExecutors" ) )
			{
			...
			else if( command.startsWith( "createProcessLinkForSame" ) )
			{
				...
				new ProcessLinkDAO(con).searchLinkedProcessList(searchResult, Process.LINK_TYPE_DEPEND, process.getId(),
				...
				ProcessAction.linkProcessCreate( con, form, linked.getSecond(), -1, null, createTypeId, "", -1 );
			}
			else if( command.startsWith( "createProcessLink" ) )
			{
				...
				ProcessAction.linkProcessCreate( con, form, process, -1, null, createTypeId, "", -1 );
			}
			else if( command.startsWith( "setRelativeDateParam" ) )
			{
				...
				if( param == null ||
					(!param.getType().equals( Parameter.TYPE_DATE ) && !param.getType().equals( Parameter.TYPE_DATETIME ) ) )
				{
			...
			else if ( command.startsWith( COMMAND_SET_PARAM ) )
			{
				...
				switch (Parameter.Type.fromString( param.getType() )) {
					case DATE:
						new ParamValueDAO( con ).updateParamDate( process.getId(), param.getId(), 
			...
			{
				EventProcessor.processEvent( new ProcessDoActionEvent( form, process, command ),
											 type.getProperties().getActualScriptName(), new SingleConnectionConnectionSet( con ) );
			}
		}
	}
	
	private static void emailSend(Connection con, DynActionForm form, UserEvent event, Process process, 
	                              ProcessType type, String subject, String exprText, int paramId, Set<Integer> userIds,
	...
	private static ProcessGroup getProcessGroup( Process process, Set<Integer> groupIds )
    ...
    public static ProcessGroup getProcessGroup( Process process, Set<Integer> groupIds, int roleId )
		throws BGMessageException
	{
		ProcessGroup processGroup = null;
    	...
    	throws BGException
    {
    	Map<String, Object> context = 
    			DefaultProcessChangeListener.getProcessJexlContext(new SingleConnectionConnectionSet(con), 
    					form, event, process);
