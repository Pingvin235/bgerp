	...
	extends Cache<ParameterCache>
{
			...
			if( listValues == null )
			{
				...
				if( Utils.notBlankString( innerJoinFilter ) )
				{
					...
					else
					{
						...
						joinFilterQuery.append( " INNER JOIN " + joinFilterTableName +
												" ON dir.id=" + joinFilterTableName + "." + tokens[1] + " AND " + tokens[2] );
					}
				}
				...
				Runnable paramExtractor = new Runnable()
				{
					...
					public void run()
					{
						...
						try
						{
							...
							Set<Integer> availableValues = Utils.toIntegerSet( param.getConfigMap().get( Parameter.LIST_PARAM_AVAILABLE_VALUES_KEY, "" ) );
	...
	//FIXME: Костыль, сделать возможность работы с #getListParamValues, т.к. принцип везде один.
	/*public static Map<Integer, String> getListCountParamValues( final Parameter param )
	{
		Map<Integer, String> result = new HashMap<Integer, String>();
		for( IdTitle value : getListParamValues( param ) )
		{
			result.put( value.getId(), value.getTitle() );
		}
	...
	//private Map<Integer, Map<Integer, String>> listCountParamValues;
