...
public class PatternFormatter
{
	...
	/**
	 * Выполняет в исходном шаблоне подстановку значений в указанные места. Если значение пустое - то скобка просто удаляется из шаблона.
	 * Для корректной обработки шаблона необходимо последовательно подставить в шаблон все возможные ключи.
	...
	private static final Pattern universalVarPattern = Pattern.compile( BEFORE_VAR + "([\\w:]+)" + AFTER_VAR );
	...
	public static String processPattern( String pattern, PatternItemProcessor processor )
	{
		...
		Matcher m = universalVarPattern.matcher( pattern );
		while( m.find() )
		{
