...
public class ConnectionPool
{
	...
	public ConnectionPool( String name, ParameterMap prefs )
	{
		...
		try
		{
			...
			for( String slaveId : prefs.parseObjectsNoOrder( "db.slave." ).keySet() )
			{
			...
			for( String trashId : prefs.parseObjectsNoOrder( "db.trash." ).keySet() )
			{
	 ...
	 * @throws Exception
	 */
	private GuardSupportedPool initConnectionPool( ParameterMap prefs, String prefix )
		throws Exception
	{
		GenericObjectPool<Connection> connectionPool = null;

		String dbDriver = prefs.get( "db.driver", null );
		...
		if( slavePools.size() > 0 )
		{
			try
			{
					...
					if( prefPool != null )
					{
						...
						if( prefPool.isOverload() )
						{
							...
							if( AlarmSender.needAlarmSend( key, time, 30 * 1000 ) )
							{
								...
								AlarmErrorMessage alarm = new AlarmErrorMessage( key, "Достигнут лимит одновременных подключений к Slave базам", message );
								AlarmSender.sendAlarm( alarm, time );
							}
				...
				catch( Exception ex )
				{
					...
					{
						String message =
							"Ошибка соединения с Slave базой данных: " + prefId + ". " +
	...
	public final Connection getSlaveConnectionFromPool( String poolId )
	{
			...
			if( poolId != null )
			{
				...
				if( pool != null )
				{
					...
					if( !isReplicationAvailable( poolId ) )
					{
						...
						{
							String message =
								"Восстановлено соединение с Slave БД с идентификатором  '" + poolId + "' ";
		...
		catch( Exception e )
		{
			...
			if( isReplicationAvailable( poolId ) )
			{
				...
				{
					String message =
						"Ошибка соединения с Slave базой данных: " + poolId + ". " +
