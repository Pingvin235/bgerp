...
extends CommonDAO
{
	...
	private static final Logger log = Logger.getLogger( FileDataDAO.class );
	...
	{
		super( con );
		storeDir = Utils.createDirectoryIfNoExistInWorkDir( Setup.getSetup().get( "file.store.dir", "filestorage" ) );
	}

	public static FileData getFromRs( ResultSet rs, String prefix, boolean loadAdditionalParameters )
	throws BGException
	{
		...
		try
		{
			...
			if( loadAdditionalParameters )
			{
				...
				if( userId > 0 ) result.setUser( UserCache.getUser( userId ) );
	...
	public FileOutputStream add( FileData file )
	throws BGException, FileNotFoundException
	{
		checkDir();
