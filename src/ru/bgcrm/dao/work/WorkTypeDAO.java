	...
	extends CommonDAO
{
		...
		throws BGException
	{
		...
		try
		{
			if( workType.getId() > 0 )
			{
				String query = " UPDATE " + ru.bgcrm.dao.work.Tables.TABLE_WORK_TYPE + " SET category=?, title=?, comment=?, config=?, non_work_hours=?, rule_config=? WHERE id=? ";
			...
			{
				String query = " INSERT INTO " + ru.bgcrm.dao.work.Tables.TABLE_WORK_TYPE + " SET category=?, title=?, comment=?, config=?, non_work_hours=?, rule_config=? ";
