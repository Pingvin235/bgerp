	...
	extends CommonDAO
{
	 ...
	 * @throws BGException
	 */
	public void searchProcessType( SearchResult<ProcessType> searchResult, int parentId, Boolean archive, String filterLike )
		throws BGException
	{
		try
		{
	 ...
	 * @throws BGException
	 */
	public List<ProcessType> getFullProcessTypeList()
		throws BGException
	{
		try
		{
		...
		throws BGException
	{
		try
		{
			...
			query =
				"SELECT status.* FROM " + TABLE_PROCESS_STATUS_TITLE + " AS status " +
				"WHERE status.id IN ( " + Utils.toString( type.getProperties().getStatusIds(), "-1", "," ) + ") " +
				"ORDER BY pos";
	 ...
	 * @throws BGException
	 */
	public void updateProcessType( ProcessType processType, int userId )
		throws BGException
	{
		try
		{
			...
			{
                ps = con.prepareStatement( "INSERT INTO " + TABLE_PROCESS_TYPE + " SET title=?, archive=?, parent_id=?, use_parent_props=?, data=?, config=?, last_modify_user_id=?, last_modify_dt=NOW()", PreparedStatement.RETURN_GENERATED_KEYS );
	...
	private ProcessType getTypeFromRs( ResultSet rs, boolean loadFull )
		throws SQLException
	{
		...
		throws BGException
	{
		try
		{
			...
			ps = con.prepareStatement( "select count(*), t1.parent_id from " + TABLE_PROCESS_TYPE + " as t1 join " + TABLE_PROCESS_TYPE + " as t2 where t2.id = ? AND t1.parent_id = t2.parent_id" );
			ps.setInt( 1, chilsId );
			rs = ps.executeQuery();
			int count = 0;
			int parentId = 0;
	 ...
	 * @return
	 */
	public boolean checkType( int id, int parentId, String title )
		throws BGException
	{
		try
		{
