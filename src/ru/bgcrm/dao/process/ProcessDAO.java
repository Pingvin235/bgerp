...
import ru.bgcrm.event.process.queue.QueueColumnAddEvent;
...
public class ProcessDAO extends CommonDAO {
	...
	private void addFilters(Queue queue, DynActionForm form, QueueSelectParams params) {
		...
		for (Filter f : filterList.getFilterList()) {
			...
			if ("groups".equals(type)) {
				...
				} else if ("close".equals(openCloseFilterValue)) {
	...
	private void addColumn(ParameterMap col, StringBuilder selectPart, StringBuilder joinPart) throws Exception {
		...
		// TODO: Сюда же можно перенести макросы выбора наименования типа, статуса и т.п., т.к. их можно выбрать из справочников
			...
			StringBuilder eventSelectPart = new StringBuilder();
			StringBuilder eventJoinPart = new StringBuilder();

			EventProcessor.processEvent(new QueueColumnAddEvent(col, eventSelectPart, eventJoinPart),
					new SingleConnectionConnectionSet(con));

			if (Utils.isBlankString(eventSelectPart.toString()) && Utils.isBlankString(eventJoinPart.toString())) {
				log.error("Incorrect column value macros: " + value);
				return;
			} else {
				selectPart.append(eventSelectPart);
				joinPart.append(eventJoinPart);
			}
