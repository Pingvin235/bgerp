	...
	extends CommonDAO
{
	...
	public void updateFilter( SavedCommonFiltersConfig commonConfig, int queueId )
		throws SQLException
	{
		...
		for( SavedFilter filter : commonConfig.getQueueSavedCommonFilterSetsMap( queueId ) )
		{
			...
			if( id < 0 )
			{
				ps = con.prepareStatement( "INSERT INTO " + TABLE_PROCESS_COMMON_FILTER + " (queue_id, title, url) VALUES (?,?,?) ", PreparedStatement.RETURN_GENERATED_KEYS );
				ps.setInt( 1, queueId );
