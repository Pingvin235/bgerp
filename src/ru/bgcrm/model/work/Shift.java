	...
	extends IdTitle
{
	...
	{
		//Collections.sort( workTypeTimeList );
	...
	public String serializeToData()
	{
		...
		for( WorkTypeTime item : workTypeTimeList ) 
		{
			...
			//TODO: Поддержка оригинального формата хранения для старой ЦРМки. Убрать в дальнейшем.
			Utils.addSetupPair( result, "", "rule." + index + ".timeFrom", "01/01/2010 " + WorkTypeTime.minutesToHourMin( item.getDayMinuteFrom() ) );
			Utils.addSetupPair( result, "", "rule." + index + ".timeTo", "01/01/2010 " +  WorkTypeTime.minutesToHourMin( item.getDayMinuteTo() ) );
	...
	/*public int getFullDuration() 
	{
		...
		for( WorkTypeTime workTypeTime : workTypeTimeList ) 
		{
			result+=workTypeTime.getDurationInHours();
		}
