	...
	extends Config
{
	...
	public SavedFiltersConfig( ParameterMap setup )
	{
		...
		for( Map.Entry<Integer, ParameterMap> me : setup.subIndexed( QUEUE_SAVED_FILTER_SET_PREFIX  ).entrySet() )		
		{
			...
			if( id <= 0 || set.getQueueId() <= 0 || 
				Utils.isBlankString( set.getUrl() ) ||
				Utils.isBlankString( set.getTitle() ) )
			{
				continue;
			}
	...
	public Map<Integer, SavedFilterSet> getTopFilters() {
    	...
    	savedFilterSetMap
    			.values().stream()
    			.filter( SavedFilterSet::isStatusCounterOnPanel )
	...
	public void updateConfig( Preferences userConfig )
	{
		...
		for( Map.Entry<Integer,List<SavedFilterSet>> me : queueSavedFilterSetsMap.entrySet() )
		{
			...
			userConfig.set( QUEUE_SAVED_FILTER_SET_ORDER + queueId, Utils.getObjectIds( setList ) );
			for( SavedFilterSet set : setList )
			{
				final String setPrefix = QUEUE_SAVED_FILTER_SET_PREFIX + set.getId();
				
				userConfig.set( setPrefix + ".queueId", String.valueOf( set.getQueueId() ) );
				userConfig.set( setPrefix + ".title", set.getTitle() );
				userConfig.set( setPrefix + ".queueName", set.getQueueName() );
				userConfig.set( setPrefix + ".url", set.getUrl() );	
				userConfig.set( setPrefix + ".rare", String.valueOf( set.isRare() ) );
				userConfig.set( setPrefix + ".statusCounterOfPanel", String.valueOf( set.isStatusCounterOnPanel() ) );
				userConfig.set( setPrefix + ".color", set.getColor() );
			}
		}
	}
