...
[snippet, from="ec", to="}"]
link:../../build.gradle#L21-L27[build.gradle]

After adding new libraries in `build.gradle` create an empty `build/changes.lib.txt` file.
...
[snippet, from="fun", to="}"]
link:../../webapps/js/kernel.ajax.js#L416-L420[webapps/js/kernel.ajax.js]
...
[IMPORTANT]
====
Public SSH key or the developer might be added for *bgerp-cdn@bgerp.org* and *www@bgerp.org*.
Before you publish a change update, make sure that `ssh bgerp-cdn@bgerp.org` and `ssh www@bgerp.org` sessions work for you.
====
