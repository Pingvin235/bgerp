...
[[lang]]
== Languages
The project currently is in migration process from Russian to international one. 
All the software documentation, including this article, log messages will be converted to English.
Information, shown to the end used via interface have to be <<l10n, localized>>.
...
The projects mostly follows recommended formatting rules for each of used programming languages.
Intention:
[square]
* 4 whitespaces - Java;
...
However because of historical reasons many files formatted wrong, it has to be carefully fixed.
As an example on the screenshot above in Java file used tabs.
...
[[log4j]]
==== Logging
By default the running in IDE application uses <<kernel/extension.adoc#log4j, logging configuration>> from `src/log4j.properties`.
For ajusting it copy the file in the project root (will not be handled by GIT) and make needed changes.
