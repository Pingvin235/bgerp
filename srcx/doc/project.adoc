...
link:../../src/ru/bgcrm/plugin/slack/Plugin.java#L22-L24[ru.bgcrm.plugin.slack.Plugin]
...
* Plugin's <<plugin-db, DB tables>>, if present, have to start from this identifier.
* Plugin's <<action, action's>> URLs, if present, have to start from `/user|usermob|open/plugin/<identifier>`.

[[plugin-db]]
==== Database
If plugin uses <<mvc, DB>>, then plugin package can contain a script which creates or modifies tables.
`db.sql` script gets invoked during standard initialization routine.
...
<<plugin-kernel, Kernel>> plugin is different one.
...
[[plugin-kernel]]
==== Kernel
The special javadoc:org.bgerp.plugin.kernel.Plugin[kernel] plugin is responsible for keeping <<plugin-db, DB update script>>,
<<plugin-l10n, Localizations>> for application's kernel. This plugin is always enabled.
Kernel <<plugin-action, Action>> classes spreads over many packages, but definitions of them stays in *PLUGIN_PACKAGE*.
...
link:../../src/org/bgerp/plugin/kernel/struts-config.xml#L51-L54[org/bgerp/plugin/kernel/struts-config.xml]

[[action-permission]]
===== Permissions check
All action methods have to be defined within <<plugin-action, plugin packages>>.
Examples:
[square]
...
link:../../src/org/bgerp/plugin/kernel/action.xml#L147-L148[org/bgerp/plugin/kernel/action.xml]
...
[snippet, from="fun", to="}"]
link:../../webapps/js/kernel.ajax.js#L375-L380[webapps/js/kernel.ajax.js]
...
=== File upload
JSP page:
link:../../webapps/WEB-INF/jspf/admin/config/list.jsp#L69-L80[webapps/WEB-INF/jspf/admin/config/list.jsp]
