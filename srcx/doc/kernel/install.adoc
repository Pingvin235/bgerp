...
[source, sh]
----
wget https://bgerp.org/release/3.0/bgerp.zip -O /tmp/bgerp.zip &&
...
Однако в момент первоначального запуска системы предпочтительно использовать *00000 Pre-Stable Release* для получения наиболее свежей функциональности.
...
[NOTE]
====
После первичной установки системы сразу же произведите обновление на изменение 00000.
...
Update files are taken from version-containing URL, for example for version 3.0: https://bgerp.org/release/3.0/
...
[[demo]]
== Demo System
The link:https://bgerp.org/#demo[Demo System] is running on https://demo.bgerp.org with the latest <<#update-version, 00000 Pre-Stable Release>> of software
and resets to initial state every 3rd hour.
