...
They mean that not primary action IDs were used to store in DB. Nothing critical, but later <<scheduler-run, execute>> the mentioned class *CorrectPermissions* to fix it.
...
Scheduler allows periodically run Java classes, which implement interface *java.lang.Runnable*.
...
Configured tasks must be visible in menu *Administration / Run / Scheduler*

image::_res/scheduler.png[]

Scheduler is controlled by <<#config-scheduler, config option>>.

[[scheduler-run]]
=== Run Classes
In menu *Administration / Run / Run* can be executed any Java class, implementing *java.lang.Runnable*.
