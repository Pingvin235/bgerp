...
When *Wait of execution is done* is selected to Yes, execution logs can be obtained from enabled <<interface.adoc#user-session-log, User Session Log>>.
...
If a toggle selected, task is running in an request thread, allowing to follow <<interface.adoc#user-session-log, logs>>.
