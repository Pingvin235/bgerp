...
* *up* or *userParam* - object of class javadoc:org.bgerp.dao.expression.ParamExpressionObject[] - параметры текущего пользователя;
* *p* or *process* - объект of class javadoc:ru.bgcrm.model.process.Process[] - изменяющийся процесс;
* *pp* or *processParam*  - object of class javadoc:org.bgerp.dao.expression.ParamExpressionObject[] - параметры изменяющегося процесса;
* *pl* or *processLink* - object of class javadoc:org.bgerp.dao.expression.ProcessLinkExpressionObject[] - привязки изменяющегося процесса;
...
* object of class javadoc:org.bgerp.dao.expression.ProcessChangeExpressionObject[] передаётся как контекст функций по-умолчанию, т.е. все его функции можно вызывать в скрипте без префикса параменной с точкой.
...
Respectively plugin Java classes have to be stored under paths: `custom/src/org/bgerp/plugin/custom/<PLUGIN_ID_AFTER_DOT>`, e.g. `custom/src/org/bgerp/plugin/custom/bitel`.

That code has equal possibilities as the native application's, can use API and connected libraries.
After compilation *Administration / Custom* the bytecode is persisted to `lib/app/custom.jar`.

image::_res/custom/custom.png[width="800"]

The custom plugins can named without *custom.* prefix and their Java code may be placed to other then *org.bgerp.plugin.custom* packages,
but in this case the custmo classes are not dynamically reloaded after each successfull compilation.
They are taken together with built-in Java classes only during the app's startup, so there is a *Restart* button in *Custom* tool available for that.
...
We offer for our Subscribers free hosting of their Custom GIT on our GitLab server.
Please, contact your Consultant for creating your GitLab accounts and making such a fork of https://git.bgerp.org/bgerp/custom/bgerp-custom

image::_res/custom/git_fork_create.png[width="800"]

image::_res/custom/git_fork_create_options.png[width="800"]

[[custom-git-workflow]]
==== Workflow
Use *bgerp-custom-<YOUR_COMPANY_NAME>* instead of *bgerp-custom-company*.

[[custom-git-workflow-app]]
===== App
You have to upload your *Public* key link:https://git.bgerp.org/-/user_settings/ssh_keys[here].
Typically you can find the Linux user's SSH key in link:https://docs.gitlab.com/user/ssh/#see-if-you-have-an-existing-ssh-key-pair[file] `~/.ssh/id_(rsa|ed25519|ecdsa).pub`
Case there is none exists yet, link:https://docs.gitlab.com/user/ssh/#generate-an-ssh-key-pair[generate] it.
...
NOTE: If you already have content in your `bgerp/custom` directory, move it to some different name first, for example `bgerp/_custom_bak`

Once you did changes, for instance, copied there the existent content from `_custom_bak`, run the commands for pushing them to your GIT repo.
[source, sh]
----
git add . &&  git commit -m "My changes in App Custom" && git push
----

[[custom-git-workflow-ide]]
===== IDE
Content of the `custom` directory may be stored using GIT and developed in full-featured IDE.
First you have to check the BGERP project out <<../project/index.adoc#checkout-reader, from GitHub>>.
After make it recognized and compiled in one of supported <<../project/index.adoc#ide, IDE>>.
...
image::_res/custom/ide.png[width="800"]
...
The easiest workflow, described above, uses only a singe *master* branch. But you can also use the same <<../project/workflow.adoc#change, GIT workflow>>
as for the main project's code. Any change there is placed to a separated branch.
