...
|title
|<<index.adoc#setup-type-title, Process Title>>
|Yes
|No
|Yes
...
(выражения можно более коротко оформить с помощью <<../iface.adoc#user-config-multiline, многострочных значений переменных>> конфигураций с JEXL выражениями):
...
* *<colId>* - the <<setup-column, column>> ID, used in the mode;
...
Or the fixed one, used as example in <<../iface.adoc#mobile, Mobile>> interface:
----
sort.combo.<comboId>.value=<fixedModeId>
----

Here is the sample of sort configuration:
[snippet, from="sort.mode.1", to="default=2"]
link:../../../itest/org/bgerp/itest/kernel/process/ProcessTest.process.queue.config.txt#L46-L57[Configuration of Kernel Process queue]

And how does it like in UI. By default sorting is first performed by descending priority and after by decending creation time.

image::_res/queue/sort.png[width="800"]
...
* *<class>* - динамический либо обычный класс, расширяющий javadoc:org.bgerp.app.event.iface.EventListener[]
...
[[processor-open]]
==== Open Interface
For <<../iface.adoc#open, open>> interface definition of processor is easier, example:
----
...
Действия *Ещё - Фильтр - Счётчик на панель* / *Фильтр - Счётчик убрать с панели* позволяют выделить число <<../iface.adoc#, счётчика>> на панель задач BGERP
