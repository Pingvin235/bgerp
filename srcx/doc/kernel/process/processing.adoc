...
* *<className>* - <<../extension.adoc#custom-src-java, Custom>> class implementing javadoc:org.bgerp.app.event.iface.EventListener[] and performing all the logic;
...
Значительная часть команд ядра может быть заменена вызовами функций из javadoc:org.bgerp.dao.expression.ProcessChangeExpressionObject[].
====
...
CAUTION: Устарел, использовать javadoc:org.bgerp.dao.expression.ProcessChangeExpressionObject[].
