...
* *<emailMessageTypeId>* - <<../../msg/email/index.adoc#setup-message-type, message type ID>> of type E-Mail which will be used for creation of a notification mail with a new license file, exact ID value may be replaced to *0* that will take the first message type of type E-Mail from server configuration;
