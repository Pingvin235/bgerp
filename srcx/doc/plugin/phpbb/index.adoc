...
== Конфигурация
Каждый интегрируемый форум должен быть настроен отдельным <<../../kernel/message.adoc#type, типом сообщения>>. Сообщение считается входящего типа.
...
onProcessEvent.2.commands=<<END
   ...
   emailNotifyExecutors(21, "Новые сообщения в процессе", "");
END  
----
