...
----
docker exec bgerp-devd cat /opt/bgerp/bgerp.properties | bash -c "sed 's#127.0.0.1/#127.0.0.1:3388/#'" | bash -c "sed 's/^#title/title/'" > bgerp.properties &&
...
link:../../../src/org/bgerp/plugin/kernel/db.sql#L304-L304[src/org/bgerp/plugin/kernel/db.sql]
...
link:../../../src/org/bgerp/plugin/kernel/db.sql#L252-L252[src/org/bgerp/plugin/kernel/db.sql]

[[cache]]
===== Cache
Caching is used within internal Map and List in order to significantly (100s of times) expedite directory data retrieval when <<jsp, building UI>> or when running <<../kernel/extension.adoc#jexl, JEXL scripts>>
...
[[plugin-ui-menu]]
===== Menu items
Menu item in <<../kernel/iface.adoc#user, user>> interface.
...
`action.xml` with <<action-permission, permission tree>> for <<../kernel/iface.adoc#user, user>> interface actions has to be located in *PLUGIN_PACKAGE*.

JSP templates:
[square]
* <<../kernel/iface.adoc#user, user>> interface in `webapps/WEB-INF/jspf/user/plugin/<PLUGIN_ID>` or `webapps/WEB-INF/jspf/admin/plugin/<PLUGIN_ID>`
* <<../kernel/iface.adoc#open, open>> interface in `webapps/WEB-INF/jspf/open/plugin/<PLUGIN_ID>`
...
[[mvc-iface]]
==== Interfaces
There are three user <<../kernel/iface.adoc#, interfaces>> exist in the program.
...
*user* <<../kernel/iface.adoc#user, interface>> actions are divided into /user and /admin
...
[[action-permission]]
===== Permissions check
All action methods for <<../kernel/iface.adoc#user, user>> interface have to be defined within `PLUGIN_PACKAGE/action.xml` files.
Examples:
[square]
...
link:../../../src/org/bgerp/plugin/kernel/action.xml#L323-L324[org/bgerp/plugin/kernel/action.xml]
...
[snippet, from="<item tit", to="te\"/>", remove-leading="		"]
...
[snippet, from="<item tit", to="em>", remove-leading="	"]
...
link:../../../webapps/WEB-INF/jspf/user/message/process/list/list.jsp#L249-L249[webapps/WEB-INF/jspf/user/message/process/list/list.jsp]

[[jsp-tags-top-line]]
====== shell:title and shell:state
Set the left area and the right areas of <<../kernel/iface.adoc#user-shell-topline, top line>> in user interface.
...
link:../../../webapps/js/kernel.ajax.js#L589-L593[webapps/js/kernel.ajax.js]
...
The key of the localizing phrase is the first entry in any language, for example English:
[source, xml]
----
<p><en>Re-login is required</en><ru>Требуется повторный вход</ru></p>
----
...
link:../../../src/org/bgerp/action/MessageAction.java#L243-L243[org.bgerp.action.MessageAction]
...
[snippet, from="if (d", to="String());", remove-leading="                "]
link:../../../src/org/bgerp/plugin/bil/subscription/dao/SubscriptionDAO.java#L87-L88[org.bgerp.plugin.bil.subscription.dao.SubscriptionDAO]
...
link:../../../src/org/bgerp/app/dist/lic/License.java#L154-L157[org.bgerp.app.dist.lic.License]

[snippet, from="final v", to="es\");", remove-leading="            "]
link:../../../src/org/bgerp/cache/UserNewsCache.java#L82-L86[org.bgerp.cache.UserNewsCache]
...
Enabling debug:
[snippet, from="$$.", to="0,"]
link:../../../webapps/js/kernel.js#L18-L24[webapps/js/kernel.js]
