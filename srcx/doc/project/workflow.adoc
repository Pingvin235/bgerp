...
[[change-git]]
=== GIT Commands
IMPORTANT: Be sure that your local GIT repo is properly <<../project.adoc#checkout-contrib, configured>>.
...
The samples of documentation's format may be found <<../samples.adoc#, here>>.
...
For building and publishing, apart from Java you will need <<../project.adoc#env-console, console environment>> with available *ant*, *ssh* and *rsync* packages.
...
NOTE: `buildUpdateLib` task checks existence of file `build/changes.lib.txt`, marking changes in <<../project.adoc#java-lib, Java libraries>>.
...
For local run use <<../project.adoc#run-data, Docker DB Instance>> for that.
