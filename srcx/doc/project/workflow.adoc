...
As examples of following the structure see plugins: <<../plugin/git/index.adoc#, GIT>>.
...
Place human readable changes description with screens in link:https://drive.google.com/drive/u/1/folders/1y2qOADOOxqi9j1WXCkdpCNLKvSBBrO9G[00000] document.
...
Pick documentation changes back to master, probably will be an empty commit.
[source, bash]
----
git checkout master && git pull --rebase && git merge --squash p11862-documentation && git commit -am "p11862 Documentation." && git push
----

Document with current changes link:https://drive.google.com/drive/u/1/folders/1y2qOADOOxqi9j1WXCkdpCNLKvSBBrO9G[00000] has to be copied to a new one.

Check if the current version and documentation time are updated on link:https://bgerp.org[Web site].
Add a news to the site link to the changes documentation.
