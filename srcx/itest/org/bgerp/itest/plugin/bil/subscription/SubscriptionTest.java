...
public class SubscriptionTest {
    ...
    public void processType() throws Exception {
        ...
        processProductTypeId = ProcessHelper.addType(TITLE + " Product", ProcessTest.processTypeTestGroupId, props).getId();

        props = new TypeProperties();
        props.setStatusIds(List.of(ProcessTest.statusOpenId, ProcessTest.statusDoneId));
        props.setCreateStatusId(ProcessTest.statusOpenId);
        props.setCloseStatusIds(Set.of(ProcessTest.statusDoneId));
        props.setConfig(ConfigHelper.generateConstants("PARAM_COST_ID", paramSubscriptionCostId, "PRODUCT_PROCESS_TYPE_ID", processProductTypeId)
                        + ResourceHelper.getResource(this, "process.subscription.type.config.txt"));
        props.setParameterIds(List.of(paramEmailId, paramSubscriptionId, paramLimitId, paramServiceCostId, paramDiscountId, paramSubscriptionCostId,
                        paramDateToId, paramLicFileId));
        processSubscriptionTypeId = ProcessHelper.addType(TITLE + " Subscription", ProcessTest.processTypeTestGroupId, props).getId();
    }
