...
<c:url var="changeOrderUrl" value="/admin/work.do">
...
<div id="${uiid}">
	...
	<form id="${groupSelectUiid}" action="/admin/work.do"  class="in-table-cell in-pr05" style="display: inline-block;">
	...
	<script>
		$(function()
		{
			...
			if( $callboard.length > 0 )
			{
				...
				var clickFunction = function()
				{
					...
					var url =
						"/admin/work.do?action=callboardUpdateShift&graphId=${form.param.graphId}" +
						"&groupId=" + groupId +
						"&userId=" + userId +
						"&date=" + date +
						"&shiftId=" + shiftId +
				...
				buttons: [ { text: "Отсортировать", click: function()
							{
								...
								// TODO: The latest usage of the deprecated function.
								openUrlToParentAsync( formUrl( $("form[action='/admin/work.do']") ), $('#${uiid}') );
								$( this ).dialog( 'destroy' );
							} },
		...
		function hideShifts()
		{
			...
			sendAJAXCommandWithParams("/admin/work.do?action=callboardUpdateFilters", {"hideEmptyShifts": hideEmptyShifts, "hideEmptyGroups": hideEmptyGroups, "graphId": graphId});
		}
	...
	<c:if test="${not empty dateSet}">
			...
			<div id="controls">
				...
				<div id="categories">
					<u:sc>
						...
						<c:set var="onSelect">
							var url = '/admin/work.do?action=callboardAvailableShift&categoryId=' + $hidden.val();
		...
		<div class="callboard">
			<table class="hdata minimal">
				...
				<tr class="header">
					<td class="shiftUser">
						<p:check action="ru.bgcrm.struts.action.admin.WorkAction:callboardGetTabel">
							<c:if test="${not empty callboard.tabelConfig}">
								<a href="/admin/work.do?action=callboardGetTabel&graphId=${form.param.graphId}&fromDate=${form.param.fromDate}&toDate=${form.param.toDate}">Табель</a>
							</c:if>
						</p:check>
					</td>
					<td><b>Час.</b></td>;
