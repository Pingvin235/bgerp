	...
	<c:otherwise>
		...
		<c:choose>
			...
			<c:when test="${editAs eq 'select'}">
				<ui:select-single list="${listValues}" value="${currentValue}" onChange="$('#${valueUiid}').val(this.value); ${changeScript}" styleClass="w100p"/>
			</c:when>
			<c:otherwise>
				<ui:combo-single value="${currentValue}" style="width: 100%;" onChange="$('#${valueUiid}').val(this.value); ${changeScript}">
