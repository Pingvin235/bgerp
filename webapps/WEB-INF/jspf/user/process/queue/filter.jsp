...
<c:set var="formAction" value="process.do"/>
...
<div id="${queue.id}" style="display:none;" class="in-inline-block" ${hideForm}>
	...
	<div style="height: 0px; max-height: 0px; width: 0px; max-width: 0px;">
		<ul style="display: none; z-index: 2000;" id="${uiidMoreMenu}">
			...
			<li draggable="true" id="savedFilters" ${hideWhenFullFilter}><a onclick="processQueueChanged( 0 );">Фильтр - вернуться в полный</a></li>
			...
			<li draggable="true" id="savedFilters" ${hideWhenFullFilter}>
				<a onclick="if( !confirm( 'Удалить сохранённый фильтр?' ) ){ return; }
					${getSavedSetId}
			    	if( sendAJAXCommand( 'process.do?action=queueSavedFilterSet&queueId=${queue.id}&id=' + savedSetId + '&command=delete' ) ){ processQueueFilterSetSelect( ${queue.id} ) }">Фильтр - удалить</a>
			</li>
			<li draggable="true" id="savedFilters" ${hideWhenFullFilter}>
				<a onclick="
					${getSavedSetId}
			    	if( sendAJAXCommand( 'process.do?action=queueSavedFilterSet&queueId=${queue.id}&id=' + savedSetId + '&command=toFullFilter' ) ){ 
			    		$('#processQueueFilter > div#${queue.id}').remove();
			    		processQueueChanged( 0 );
			...
			<li draggable="true" id="${queue.id}-0" ${hideWhenSavedFilter}><a onclick="processQueueChanged( 1 );">Фильтр - сохранённые</a></li>
			...
			<li draggable="true" id="${queue.id}-0" ${hideWhenSavedFilter}>
				<a onclick="if( !confirm( 'Сбросить полный фильтр?' ) ){ return; }
					if( sendAJAXCommand( 'process.do?action=queueSavedFilterSet&queueId=${queue.id}&id=0&command=toFullFilter' ) ){ 
			    		$('#processQueueFilter > div#${queue.id}').remove();
			    		processQueueChanged( 0 );
			    	}">Фильтр - сброс</a>
			</li>
	...
	<div style="display: inline-block;">
		<c:forEach var="processor" items="${queue.processorMap.values()}">
			<form action="process.do" id="${addActionFormUiid}-${processor.id}" style="display: none;" class="in-mb1-all">
				...
				<c:set var="doScript">
					...
					{
						if( sendAJAXCommand( formUrl(this.form) + debug ) ){ ${sendCommand} }	
					}
	...
	<%-- сохранение фильтра --%>	
	<form action="process.do" style="display: none;" id="${saveFilterFormUiid}" class="mb1 mr1">
		Название фильтра:
		...
		<button type="button" class="btn-grey" 
			onclick="if( this.form.title.value == '' ){ alert( 'Введите название!'); return; } 
					 ...
					 if( sendAJAXCommand( formUrl( this.form ) ) ){ processQueueFilterSetSelect( ${queue.id} ) }">OK</button>
		<%--$('#${uiid} #buttons' ).show();  --%>			  
	...
	<div id="savedFilters" ${hideWhenFullFilter} class="in-mb1-all mr1 in-mr05 dropFilterArea">
		<c:forEach var="saved" items="${config.queueSavedFilterSetsMap[queue.id]}">
			<form action="${saved.url}" id="${queue.id}-${saved.id}" ${currentSavedFilterSetId eq saved.id ? "active='1'" : ""} style="display: none;">
				<input type="hidden" name="savedFilterSetId" value="${saved.id}"/>
			</form>
				<div draggable="true" id="${saved.id}" class="${currentSavedFilterSetId eq saved.id ? 'btn-blue' : 'btn-white'}" onclick="processQueueChanged( ${saved.id} )">${saved.title}</div>
	...
	<%-- полный фильтр --%>
	<form action="process.do" id="${queue.id}-0" ${currentSavedFilterSetId le 0 ? "active='1'" : "style='display: none;'"} class="in-inline-block in-mr05 in-mb1-all">
		...
		<u:sc export="filters">
			<c:set var="valuesHtml">
				<c:forEach var="filterFromList" items="${queue.filterList.filterList}">
					...
					<c:if test="${filterFromList.type == 'openClose'}">
						...
						<c:set var="title" value="Открыт - закрыт"/>
						<c:set var="code">
							<u:sc>
			...
			<c:set var="onChange">
				...
				uiComboInputs($("#${id}")).each( function() {
					if( this.checked )
					{
