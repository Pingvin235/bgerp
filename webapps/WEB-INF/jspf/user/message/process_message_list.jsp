		  	...
		  	<tr style="background-color: ${color}; ${style}">
				...
				<td width="30px" style="white-space: nowrap;">
					...
					<div style="height: 0px; max-height: 0px; width: 0px; max-width: 0px; display: inline-block;">
						<ul id="${menuUiid}" style="display: none;">
							<c:set var="command">
							...
							<li><a href="#UNDEF" onclick="$('#${linkFormUiid}').css('display', 'inline-block'); return false;">Изменить процесс</a></li>
							...
							<c:if test="${messageType.processChangeSupport}">
								<c:set var="command">
									if(confirm('Привязать сообщение к вновь созданной копии текущего процесса?')) {
										<c:url var="url" value="message.do">
										...
										if(result) {
											openProcess(result.data.process.id);
											var $tabs = $('div#process-' + result.data.process.id + ' #processTabsDiv' );
											$tabs.tabs( 'showTab', 'process-messages' );
										}
									}
									return false;
								</c:set>
								<li><a href="#UNDEF" onclick="${command}">Изменить процесс на копию</a></li>
							</c:if>	
							...
							<c:if test="${message.direction eq 1 and messageType.answerSupport}">
								...
								<%-- <c:set var="answerText" value="${fn:replace( answerText, singleQuot, '\\\\\\'' )}"/>  --%>
								...
								<%-- <c:set var="answerText" value=">${fn:escapeXml( answerText )}"/> --%>
