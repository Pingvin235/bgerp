...
<%@page import="ru.bgcrm.struts.action.WorkAction.ShiftData"%>
...
<div id="${uiid}">
	...
	<form id="${groupSelectUiid}" action="/user/work.do"  class="in-table-cell in-pr05" style="display: inline-block;">
		...
		<div>
		 ...
		 <div>		
		 	<button type="button" class="btn-grey ml05" onclick="openUrlToParent( formUrl( this.form ), $('#${uiid}') )" title="Вывести">=&gt;</button>
		 </div>	
	</form>
			...
			if( $tableDiv.length > 0 )
			{
				...
				var clickFunction = function()
				{
					...
					if( mode == 'lock' )
					{
						...
						{
							var result = sendAJAXCommand( "/user/work.do?action=processTimeLock" + urlEnd );
							if( result )
							{
					...
					else if( mode == 'unlock' )
					{
						if( $(this).hasClass( "lock" ) )
						{
							var result = sendAJAXCommand( "/user/work.do?action=processTimeUnlock" + urlEnd );
							if( result )
							{
								$(this).toggleClass( "lock" );
								$(this).html( "" );
