...
<shell:state text="${l.l('Subscription Debts')}"/>
...
<div class="report center1020">
	...
	<div class="data mt1 w100p" style="overflow: auto;">
		<c:choose>
			<c:when test="${not empty subscriptionUserAmounts}">
				<h1>${l.l('User')}</h1>
								...
								<c:forEach var="r" items="${frd.list}">
									<c:if test="${r.get('subscription_id') eq subscription.id}">
										<tr>
											...
											<td><c:if test="${newProcessId}"><ui:user-link id="${r.get('payment_user_id')}"/></c:if></td>
											<td><c:if test="${newProcessId}">${r.get('payment_user_tax')}</c:if></td>
											...
											<td>
												<c:set var="productProcessId" value="${r.get('product_id')}"/>
												<c:if test="${productProcessId gt 0}">
													<ui:process-link id="${productProcessId}" text="${r.get('product_description')}"/>
												</c:if>
											</td>
			...
			<c:otherwise><c:if test="${subscriptionUserAmounts ne null}">No Data</c:if></c:otherwise>
		</c:choose>
	</div>
</div>