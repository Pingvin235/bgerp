...
<table width="100%" class="hdata">
	<tr class="header">
		<td colspan="6">Ошибки логина ${form.param.login} за последние ${form.param.errorDays} суток. 
			...
			<form action="/user/plugin/bgbilling/proto/dialup.do">
				...
				<select name="errorDays" onchange="openUrlTo(formUrl( this.form), $('#${form.param.uiid}sessionErrorList') );scrollToElementById('${form.param.uiid}sessionErrorList');">
	...
	<c:forEach var="dialUpError" items="${form.response.data.errorList}">
			<tr align="center">
				...
				<td>									
					<input type="button" style="width: 100%" value="Show Log" onclick="if ($('#${form.param.contractId}-${dialUpError['id']}-radiusLog').children().size()<=0 ) { openUrlTo('${radiusLogUrl}', $('#${form.param.contractId}-${dialUpError['id']}-radiusLog')  ); } else { $('#${form.param.contractId}-${dialUpError['id']}-radiusLog').toggle(); }" />
				</td>
			</tr>
