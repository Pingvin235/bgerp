...
<div id="${listUiid}">
	<c:if test="${empty onlyData}">
		<form action="/user/plugin/bgbilling/proto/contract.do" class="mb1 in-mr1" style="display: inline-block;">
			...
			<ui:combo-single name="showEmptyParameters" value="${form.param.showEmptyParameters}" prefixText="Только заполненные:" widthTextValue="2em"
				onChange="$$.ajax.load(this.form, $('#${listUiid}'));">
			...
			<ui:combo-single name="onlyFromGroup" value="${form.param.onlyFromGroup}" prefixText="Только из группы:" widthTextValue="2em"
				onChange="$$.ajax.load(this.form, $('#${listUiid}'));">
				<jsp:attribute name="valuesHtml">
					<li value="1">Да</li>
					<li value="0">Нет</li>
	...
	--%><form action="/user/plugin/bgbilling/proto/contract.do" class="mb1 ml1" style="display: inline-block;">
			...
			<ui:combo-single list="${frd.group.values}" name="paramGroupId" value="${frd.group.id}" prefixText="Группа параметров (изменить):"
				widthTextValue="12em" onChange="$$.ajax.post(this.form).done(() => $$.ajax.load('${form.requestUrl}', $('#${listUiid}')))"/>
		</form>
	</c:if>
								...
								<c:set var="viewEditDivId" value="${u:uiid()}"/>
									<div id="${viewEditDivId}">
										<form action="/user/plugin/bgbilling/proto/contract.do">
											...
											<a href="#" onclick="$$.ajax.load($(this).parent(), $('#${viewEditDivId}')); return false;">
												<c:choose>
													<c:when test="${not empty contractParameter.value}">${u.escapeXml(contractParameter.value)}</c:when>
