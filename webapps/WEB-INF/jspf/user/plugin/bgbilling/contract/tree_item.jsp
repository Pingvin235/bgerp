...
<c:set var="rowAttrs">id="${rowId}" class="${rowClass}"</c:set>
 ...
 <c:choose>
	...
	<c:when test="${not empty url}">
		<c:url var="billingActionUrl" value="plugin/bgbilling/proto/${url}">
			<c:param name="billingId" value="${contract.billingId}"/>
			<c:param name="contractId" value="${contract.id}"/>
		</c:url>	
		<tr style="cursor: pointer;" onClick="markOutTr(this); openUrlTo('${billingActionUrl}', $('#${contractTreeId} #content'));" ${rowAttrs}>						
	</c:when>
	<c:otherwise> 
		<tr ${rowAttrs}>
	</c:otherwise>
