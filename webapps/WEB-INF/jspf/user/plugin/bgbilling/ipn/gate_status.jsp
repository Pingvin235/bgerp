		...
		<html:form action="/user/plugin/bgbilling/proto/ipn" style="width: 100%;">
			...
			<u:sc>
				<c:set var="valuesHtml">
					...
					<li value="3">Жесткая блокировка</li>
					<li value="4">Удален</li>
				</c:set>
				<c:set var="hiddenName" value="status"/>
				<c:set var="value" value="${info.statusId}"/>
				<c:set var="style" value="width: 100%;"/>
				<c:set var="onSelect" value="if( sendAJAXCommand( formUrl( $('#${uiid}').find('form')[0] ) ) ){ $$.ajax.load('${form.requestUrl}', $('#${uiid}').parent()) }"/>
