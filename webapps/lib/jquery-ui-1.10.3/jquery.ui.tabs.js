...
$.widget( "ui.tabs", {
	...
	_create: function() {
		...
		//PATCH BGERP, sync load and cache loaded
		...
		if ( this.xhr && this.xhr.statusText !== "canceled" ) {
			...
			this.xhr
				.success(function( response ) {
					/* BGERP FIX не нужна поддержка jQuery < 1.8, а вот по сути асинхронная загрузка мешала,
					 * т.к. не возможно было после вызова tabs() сразу оперировать с табами.
					 * 
					 * // support: jQuery <1.8
				...
				.error(function (jqXHR, textStatus) {
					panel.html('<b>Ошибка: ' + jqXHR.responseText + '</b>');
				})
	...
	// BGERP PATCH: load ignore cache
	loadForce: function(index) {
		this.tabs.eq( index ).data("loaded", false);
		this.load(index);
	},
	...
	//PATCH ADDITION BGERP, restore function add 
	...
	//PATCH ADDITION BGERP, restore function remove
